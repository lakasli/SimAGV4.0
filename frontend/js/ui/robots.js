...
function updateRobotList() {
  ...
  c.innerHTML = registeredRobots.map(robot => {
    const id = String(robot.robot_name || '').trim();
    ...
    const instColor = instStatus === '启动' ? '#00ff00' : (instStatus === '断开' ? '#ff0000' : '#ff0000');
    ...
    return `<div class="robot-item" style="${selectedStyle}" data-action="select-robot" data-robot-id="${escapeHtml(id)}">
